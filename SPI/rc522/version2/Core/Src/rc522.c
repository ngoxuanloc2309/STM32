...
//	serNum[1] = 0x20;
////	u8 sendData[2] = {PICC_ANTICOLL, 0x20};
...
//		for (i=0; i<4; i++)
//		{   
//		 	serNumCheck ^= serNum[i];	// Giao thuc MIFARE
//		}
...
u8 rc522_anticol(u8 *serNum){
    ...
    u8 sendData[2] = {PICC_ANTICOLL, 0x20};
    u8 recvData[5];
    status = rc522_to_card(ControlReg, sendData, 2, recvData, &unLen);

    if (status == 0)
    {
        if (unLen != 40) // 5 byte
        {
            status = 2; // loi ko nhan du byte
        }
        else
        {
            for (i=0; i<4; i++)
            {   
                serNumCheck ^= recvData[i];
            }
            if (serNumCheck != recvData[i])
            {   
                status = 2;    
            }
            else
